...
public class Confs_web_Service extends SqlToolseService {
    ...
    public DataMap getConfsList(String token, Integer state, String select, Integer page, Integer display) throws EhCacheSessiconException {
        ...
        System.out.println("1where.toString()="+where.toString());
	 ...
	 * @throws EhCacheSessiconException 
     */
    public Map<String, Object> getConfsListPc(String token, Integer state, String select, Integer page, Integer display,Users users) throws EhCacheSessiconException {
        ...
        // 查询字段
        StringBuffer find = new StringBuffer(" obj.id, obj.cname, obj.brief, obj.addr,");
        find.append(" obj.cimg, obj.bhstart, obj.bhend ,ct.tname AS type,obj.shstart,obj.shend ");
        // 条件
        StringBuffer where = new StringBuffer("1=1 AND obj.state=1 and obj.isdelete=1 AND obj.userid=" + users.getId());
        if (state == 1) {//1正在进行 
            where.append(" AND NOW() BETWEEN obj.bhstart AND obj.bhend ");
        ...
        //排序
        String order = " obj.createtime DESC";
        System.out.println("2where.toString()="+where.toString());
//        System.out.println(""+table_name.toString()+"===="+ find.toString()+"===="+ 
//                where.toString()+"==222=="+  order+"===="+  page+"===="+  display);
	 ...
	 * @return
	 */
    public DataMap getConfsPc(String id ,int type) {
        DataMap dataMap = new DataMap();
        if(id==null || id.trim().equals("")){
        ...
        if(type==2){
        	...
        	find.append(" obj.introd, obj.id, obj.cname, obj.cimg, obj.addr, obj.brief, obj.touch, obj.latitude, obj.longitude, obj.position,").
    ...
    public DataMap getConfs(String id ,int type) {
        ...
        if(type==2){
        	...
        	find.append(" obj.id, obj.cname, obj.cimg, obj.addr, obj.brief, obj.touch, obj.latitude, obj.longitude, obj.position, obj.shstart, obj.shend, ").
        		append(" DATE_FORMAT(obj.bhstart,'%Y-%m-%d')AS days, DATE_FORMAT(obj.bhstart,'%H:%i') AS statime, DATE_FORMAT(obj.bhend,'%H:%i') AS endtime,").
        		append(" DATE_FORMAT(obj.bhend,'%Y-%m-%d')AS edays, obj.type as typeid, ct.tname");
        }
        String where = " obj.id='"+id+"' "; // 条件and obj.isdelete=1
        Map<String, Object> map = this.selectMap(table_name.toString(), find.toString(), where);
        dataMap.addMsg_map(map, 0);
        return dataMap;
    }
	 ...
	 * @return
	 */
    public DataMap getConf(String id ,int type) {
        DataMap dataMap = new DataMap();
        if(id==null || id.trim().equals("")){
        	dataMap.addMsg_diy_obj(null, 6, "未找到该会议");
        	return dataMap;
        ...
        StringBuilder find = new StringBuilder(); // 显示字段
        if(type==2){
        	find.append(" obj.introd, obj.imgs,obj.shstart,obj.shend"); 
        }else{
        	find.append(" obj.id, obj.cname, obj.cimg, obj.addr, obj.brief, obj.touch, obj.latitude, obj.longitude, obj.position, obj.shstart, obj.shend, ").
        		append(" DATE_FORMAT(obj.bhstart,'%Y-%m-%d')AS days, DATE_FORMAT(obj.bhstart,'%H:%i') AS statime, DATE_FORMAT(obj.bhend,'%H:%i') AS endtime,").
        		append(" DATE_FORMAT(obj.bhend,'%Y-%m-%d')AS edays, obj.type as typeid, ct.tname");
        }
        String where = " obj.id='"+id+"' "; // 条件and obj.isdelete=2
        Map<String, Object> map = this.selectMap(table_name.toString(), find.toString(), where);
        dataMap.addMsg_map(map, 0);
        return dataMap;
    }
