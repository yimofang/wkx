...
public class ConfsBack_web_Controller {
     ...
     */
    @ResponseBody
    @RequestMapping(value = "/delete_backPc",method = RequestMethod.GET)
    public Map<String, Object> deleteConfsBackPc(@RequestParam(value = "token", required = true) String token,
    		@RequestParam(value = "confid", required = true) String confid,
            @RequestParam(value = "imgs", required = false) String imgs) {
        DataMap dataMap = new DataMap();
		try {
			dataMap = this.confsBackService.deleteConfsBackPc(token, confid, imgs);
		} catch (EhCacheSessiconException e) {
			e.printStackTrace();
			dataMap.addMsg_diy_list(null, 5, "登录超时，请重新登录");
			return dataMap.data;
