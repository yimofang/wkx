...
public class AdminAspect {
	...
	public void foundAfter(JoinPoint point, Object returnValue){
		...
		String[] m;
		try {
			m = InfoSwicth(point,controllername,returnValue);
			if(m!=null) {
				//获取用户信息
				Session shiro_session = SecurityUtils.getSubject().getSession();
				Users userinfo = (Users) shiro_session.getAttribute("admin");
	 ...
	 * @return
	 */
	private String[] InfoSwicth(JoinPoint point,String controllername,Object returnValue) throws Exception{		
		...
		if(methodName.indexOf("add")>=0) {
			if(returnValue==null || !((Map<String, Object>) returnValue).get("error").equals("0")) {
				return null;
			}
			strs[0]="添加";
			strs[3]="1";
