            ...
            success: function (res) {
                ...
                if (res.error == 0) {
                    if (res.row.shstart != '' && res.row.shend != '') {
                        ...
                        $('.title button.mNamed').addClass('active');
                        $('.cont5').show();
                        $('.cont1').hide();
                        $('.cont2').hide();
                        $('.cont3').hide();
...
function JudgeInfo() {
    var Meimg = Mimg(),
        ...
        Memap = Mmap(),
        Minfo = InfoCkeck();
    if (Meimg && Metype && Mename && Metime && Mezbf && Meaddr && Mephone && Memap && Minfo) {
        return true;
    } else if (!Meimg || !Metype || !Mename || !Metime || !Mezbf || !Meaddr || !Mephone || !Memap || !Minfo) {
...
function checkGuest() {
    ...
    for (var i = 0, isok = true; i < arr.length; i++) {
        ...
        var obj3 = {
            ...
            gname: name,
            gimg: $(".cont3 li:eq(" + i + ") .g_img img").attr('src'),
            rank: honer,
            brief: desc
        };
        data3.push(obj3);
