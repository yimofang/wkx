	...
	if(btn != null && btn != "null"){
		$.each($.parseJSON(btn),function (index,data){
			...
			if(typeStyle == "disable"){
				...
				btnHtml += 'onclick="onClickCommonForm(\''+data.text+'\','+data.form_id+',\''+typeStyle+'\','+n_id+','+flow.id+')"';
			}
...
function onClickCommonForm(title,form_id,typea,node_id,flow_id){
		$.myPlugin.modelDialog({
			context:contextPath+'/page/form/commonFlowForm.jsp',
			data:{form_id:form_id,node_id:node_id,flow_id:flow_id,type:typea},
			title:title,
			width:'700px',
			height:'600px',
			offset:'80px',
