...
function getAllRoleInfo(){//获取全部角色
	$.ajax({
		url:contextPath+"/system/role/getOrgRoleTree.do",
		type:"post",
		dataType:"json",
		...
		success:function(data){
			var obj = eval(data);
			var data = obj.data;
            $('#configOrganInfo #roleList').treeview({
                data: data,					//数据源
...
function submitOrganRoleConfig(){//提交组织机构角色
	var node = $('#configOrganInfo #roleList').treeview(true).getChecked();
	var datas = {
		"orgId":organizationId,
		"node":node
	}
	$.ajax({
		url:contextPath+"/system/role/insertOrgRoleList.do",
		type:"post",
		dataType:"json",
		data:datas,
		success:function(data){
