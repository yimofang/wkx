        ...
        success: function (res) {
            if (res.error == 0) {
                ...
                var htmlstr = `<h2>${res.row.cname}</h2>
                            <p class='preMap' data-lat='${res.row.latitude}' data-lng='${res.row.longitude}'><i class="Icon Icon1"></i><span class="ditu">${res.row.position}${res.row.addr}</span></p>
...
function Getquestion() {
    $.ajax({
        ...
        success: function (res) {
            ...
            if (res.error == 0) {
                console.log(res.row);
                if (res.row.titles == null || res.row.titles.length == 0) {
                    $("#pShow").hide();
                } else if (res.row.titles != null) {
                    $("#pShow").show();
                    var html7 = '';
...
function DACheck(index) {
    var qusid = $(".previewContent ul li:eq(" + index + ")").attr('data-id');
    // var qusval =$("input[name='check"+index+"']:checked");
    var qusval = $("input[name=" + (index + 1) + "]:checked");
    var json = {"titleid": '', "optionid": ''};
